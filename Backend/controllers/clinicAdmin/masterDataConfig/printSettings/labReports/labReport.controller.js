import LabReport from "../../../../../models/clinicAdmin/masterDataConfig/printSettings/labReports/labReport.model.js";
...
export const createDefaultPrintSettings = async (req, res) => {
    try {
        ...
        const existingSettings = await LabReport.findOne({ clinicId });
        if (existingSettings) {
        ...
        const newSettings = new LabReport({
...
export const getPrintSettings = async (req, res) => {
    try {
        ...
        const printSettings = await LabReport.findOne({ clinicId: req.user.id });
...
export const updateBasicSettings = async (req, res) => {
    try {
        ...
        const updatedSettings = await LabReport.findOneAndUpdate(
            { clinicId : req.user.id },
            {
                $set: {
...
export const updateBannerSettings = async (req, res) => {
    try {
        ...
        let existingSettings = await LabReport.findOne({ clinicId: req.user.id });
        if (!existingSettings) {
            return res.status(404).json({ message: "Print settings not found for this clinic" });
        }
...
export const addSignature = async (req, res) => {
    try {
        ...
        const prescription = await LabReport.findOne({ clinicId : req.user.id });
        if (!prescription) {
