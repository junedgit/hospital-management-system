...
export const createDefaultPrintSettings = async (req, res) => {
    try {
        ...
        const existingSettings = await ConsultationPrint.findOne({ clinicId });
        if (existingSettings) {
        ...
        const newSettings = new ConsultationPrint({
...
export const getPrintSettings = async (req, res) => {
    try {
        ...
        const printSettings = await ConsultationPrint.findOne({ clinicId: req.user.id });
...
export const updateBasicSettings = async (req, res) => {
    try {
        ...
        const updatedSettings = await ConsultationPrint.findOneAndUpdate(
            { clinicId : req.user.id },
            {
                $set: {
...
export const updateBannerSettings = async (req, res) => {
    try {
        ...
        let existingSettings = await ConsultationPrint.findOne({ clinicId: req.user.id });
        if (!existingSettings) {
            return res.status(404).json({ message: "Print settings not found for this clinic" });
        }
