import PharmacyBill from "../../../../../models/clinicAdmin/masterDataConfig/printSettings/pharmacyBill/pharmacyBill.model.js";
...
export const createDefaultPrintSettings = async (req, res) => {
    try {
        ...
        const existingSettings = await PharmacyBill.findOne({ clinicId });
        if (existingSettings) {
        ...
        const newSettings = new PharmacyBill({
...
export const getPrintSettings = async (req, res) => {
    try {
        ...
        const printSettings = await PharmacyBill.findOne({ clinicId: req.user.id });
...
export const updateBasicSettings = async (req, res) => {
    try {
        ...
        const updatedSettings = await PharmacyBill.findOneAndUpdate(
            { clinicId : req.user.id },
            {
                $set: {
...
export const updateBannerSettings = async (req, res) => {
    try {
        ...
        let existingSettings = await PharmacyBill.findOne({ clinicId: req.user.id });
        if (!existingSettings) {
            return res.status(404).json({ message: "Print settings not found for this clinic" });
        }
