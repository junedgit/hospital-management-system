import BillReceipt from "../../../../../models/clinicAdmin/masterDataConfig/printSettings/billReceipt/billReceipt.model.js";
...
export const createDefaultPrintSettings = async (req, res) => {
    try {
        ...
        const existingSettings = await BillReceipt.findOne({ clinicId });
        if (existingSettings) {
        ...
        const newSettings = new BillReceipt({
...
export const getPrintSettings = async (req, res) => {
    try {
        ...
        const printSettings = await BillReceipt.findOne({ clinicId: req.user.id });
...
export const updateBasicSettings = async (req, res) => {
    try {
        ...
        const updatedSettings = await BillReceipt.findOneAndUpdate(
            { clinicId : req.user.id },
            {
                $set: {
...
export const updateBannerSettings = async (req, res) => {
    try {
        ...
        let existingSettings = await BillReceipt.findOne({ clinicId: req.user.id });
        if (!existingSettings) {
            return res.status(404).json({ message: "Print settings not found for this clinic" });
        }
